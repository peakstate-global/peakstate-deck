...
/* ══ STATES AND FLASH ANIMATIONS ═══════════════════════════════════════════
   Added 28 August 2026. Two features that travel together because the second
   is what makes the first worth having.

   A STATE is a section carrying `data-state-group="<id>"`. Sections sharing a
   group are states of one slide: they are navigated like separate slides, they
   share one slide number, and **only the group's primary state reaches print**.
   That is what stops a nine-state build becoming nine pages in a PDF.

   `data-state-primary` marks the primary. If none is marked the first state
   takes the job. If the primary is hidden, the job falls back to the previous
   visible state in the group, then forward, so a hidden primary never leaves a
   group with no page.

   An ANIMATION is `data-entry` or `data-exit` on any element inside a slide,
   valued `flash-left` or `flash-down`. Entry runs when the slide becomes
   visible; exit runs as it leaves. `deckFlash(el, name)` fires the same
   animation at any other time, which is why the CSS is keyed off a class
   rather than off slide changes.

   ponytail: motion blur is a short filter on the moving axis, dropped as it
   lands, rather than a duplicated ghost trail. The ceiling is that it reads as
   a smear rather than a true trail; the upgrade path is a cloned element behind
   the real one, and it doubles the nodes on every animated slide.
   ══════════════════════════════════════════════════════════════════════════ */
(function () {
  var DUR = 300;                       // ms, the whole flash
  var NAMES = { 'flash-left': 'x', 'flash-down': 'y' };

  var css = document.createElement('style');
  css.id = 'deck-stage-flash';
  css.textContent = [
    /* The travel is the element's own size, so a wide thing flies further than
       a narrow one and the motion reads as the same speed on both. */
    '@keyframes deck-flash-in-x{',
    '  from{opacity:0;transform:translateX(100%);filter:blur(6px)}',
    '  70%{opacity:1;transform:translateX(-2%);filter:blur(0)}',
    '  85%{transform:translateX(1%)}',
    '  to{opacity:1;transform:translateX(0);filter:blur(0)}}',
    '@keyframes deck-flash-out-x{',
    '  from{opacity:1;transform:translateX(0);filter:blur(0)}',
    '  to{opacity:0;transform:translateX(-100%);filter:blur(6px)}}',
    '@keyframes deck-flash-in-y{',
    '  from{opacity:0;transform:translateY(-100%);filter:blur(6px)}',
    '  70%{opacity:1;transform:translateY(2%);filter:blur(0)}',
    '  85%{transform:translateY(-1%)}',
    '  to{opacity:1;transform:translateY(0);filter:blur(0)}}',
    '@keyframes deck-flash-out-y{',
    '  from{opacity:1;transform:translateY(0);filter:blur(0)}',
    '  to{opacity:0;transform:translateY(100%);filter:blur(6px)}}',
    '.deck-flash-in-x{animation:deck-flash-in-x ' + DUR + 'ms cubic-bezier(.2,.9,.2,1) both}',
    /* An exit is linear. An ease-in starts slow, and a slow start on the way out
       reads as the element loitering while the next state is already arriving. */
    '.deck-flash-out-x{animation:deck-flash-out-x ' + DUR + 'ms linear both}',
    '.deck-flash-in-y{animation:deck-flash-in-y ' + DUR + 'ms cubic-bezier(.2,.9,.2,1) both}',
    '.deck-flash-out-y{animation:deck-flash-out-y ' + DUR + 'ms linear both}',
    /* Print and reduced motion get the end state and no movement, so a PDF is
       never a page of half-faded elements. */
    '@media print{.deck-flash-in-x,.deck-flash-out-x,.deck-flash-in-y,.deck-flash-out-y{animation:none!important;opacity:1!important;transform:none!important;filter:none!important}}',
    '@media (prefers-reduced-motion: reduce){',
    '  .deck-flash-in-x,.deck-flash-out-x,.deck-flash-in-y,.deck-flash-out-y{animation-duration:1ms!important}}',
    /* A non-primary state is a screen-only frame. */
    '@media print{deck-stage > section[data-state-group]:not([data-state-page]){display:none!important}}',
    /* An exit has to be SEEN, and the stage hides a slide the instant it stops
       being active, so without this the animation plays on an invisible element
       and the reader sees a cut. The outgoing slide is held on top for the
       length of the flash, with a transparent background and everything hidden
       except the elements that are leaving. visibility is inheritable, which is
       what lets one line hide the slide and the next line bring back only the
  ...
  function isHidden(sec) {
    return sec.hasAttribute('data-hidden-src') || sec.hasAttribute('data-hidden');
  }

  /** Mark one section per group as the page that prints.
   *
   *  The marked primary wins. When it is hidden the job falls back to the
   *  previous visible state, then forward, because a group whose every state is
   *  hidden should contribute no page rather than a hidden one. */
  function markPrintPages() {
    var groups = {};
    slides().forEach(function (sec) {
      var g = sec.getAttribute('data-state-group');
      sec.removeAttribute('data-state-page');
      if (!g) return;
      (groups[g] = groups[g] || []).push(sec);
    });
    Object.keys(groups).forEach(function (g) {
      var list = groups[g];
      var marked = list.filter(function (s) { return s.hasAttribute('data-state-primary'); })[0];
      var pick = marked && !isHidden(marked) ? marked : null;
      if (!pick && marked) {
        var i = list.indexOf(marked);
        for (var b = i - 1; b >= 0 && !pick; b--) if (!isHidden(list[b])) pick = list[b];
        for (var f = i + 1; f < list.length && !pick; f++) if (!isHidden(list[f])) pick = list[f];
      }
      if (!pick) pick = list.filter(function (s) { return !isHidden(s); })[0];
      if (pick) pick.setAttribute('data-state-page', '');
    });
  }

  /** Run one flash on one element, at any time. Returns when it finishes. */
  function deckFlash(el, name) {
    var axis = NAMES[name];
    if (!el || !axis) return Promise.resolve();
    var dir = name.indexOf('out') > -1 ? 'out' : 'in';
    return runClass(el, 'deck-flash-' + dir + '-' + axis);
  }
  function runClass(el, cls, keep) {
    return new Promise(function (done) {
      el.classList.remove(cls);
      void el.offsetWidth;                        // restart a repeated animation
      el.classList.add(cls);
      // An exit KEEPS its class. The animation fills both ends, so dropping the
      // class at animationend snaps the element back to full opacity while the
      // slide is still held on top, and the reader sees it blink back in and
      // then out again. The caller drops the class after the slide is gone.
      var end = function () { if (!keep) el.classList.remove(cls); el.removeEventListener('animationend', end); done(); };
      el.addEventListener('animationend', end);
      setTimeout(end, DUR + 120);                 // never strand an element mid-flash
    });
  }

  function play(sec, attr, dir, keep) {
    if (!sec) return;
    Array.prototype.forEach.call(sec.querySelectorAll('[' + attr + ']'), function (el) {
      var axis = NAMES[el.getAttribute(attr)];
      if (axis) runClass(el, 'deck-flash-' + dir + '-' + axis, keep);
    });
  }
  function clearExit(sec) {
    if (!sec) return;
    Array.prototype.forEach.call(sec.querySelectorAll('[data-exit]'), function (el) {
      el.classList.remove('deck-flash-out-x', 'deck-flash-out-y');
    });
  }

  /* ── Morph, the default transition between states ────────────────────────
     PowerPoint's morph, and the same idea: an element that appears in both
     states does not cut, it travels. The underline under the chosen word gets
     longer for "elephant" and shorter for "mat" rather than jumping.

     Matching is by SIGNATURE, not by hand: an element's tag, classes and place
     among its siblings, walked up to the section. States are generated from one
     template, so the same element has the same signature in every state and
     nothing needs labelling. `data-morph="key"` overrides the signature where
     two elements should be treated as the same thing, and `data-morph="none"`
     opts out.

     An element carrying its own entry or exit animation is left alone, along
     with everything inside it. An explicit animation is a statement that this
     element should arrive its own way, and morphing it as well fights it.

     ponytail: FLIP by transform, plus width and height where they differ. The
     ceiling is that morphing text distorts nothing but also crossfades nothing,
     so a word that changes reads as a cut inside a moving box. Upgrade path is
     a crossfade of two clones, which doubles the nodes on every morphed
     element. */
  // Same clock as a flash, and no easing. A morph that runs longer than the
  // flash beside it, or eases while the flash does not, reads as two separate
  // moves. It is one move.
  // 210ms, not the flash's 300. A flash lands its element at 70% of its keyframes
  // and spends the rest overshooting and settling, so a linear morph on the same
  // clock is still travelling after the word has arrived, and reads as slow.
  var MORPH = 210;
  var MORPH_EASE = 'linear';
  function morphMap(sec, linked) {
    var map = {};
    if (!sec) return map;
    Array.prototype.forEach.call(sec.querySelectorAll('[data-morph]'), function (el) {
      var k = el.getAttribute('data-morph');
      if (!k || k === 'none') return;
      if (!linked && el.closest('[data-entry],[data-exit]')) return;
      map[k] = el;
    });
    return map;
  }
  function morph(from, to) {
    ...
    // States morph by default. Two ORDINARY slides do not, because moving every
    // shared element between unrelated slides is noise. `data-morph-link` on
    // either of two ADJACENT sections opts that one pair in, which is how a
    // build hands off to the slide that follows it.
    var g = from.getAttribute('data-state-group');
    var linked = false;
    if (!g || g !== to.getAttribute('data-state-group')) {
      var all = slides(), i = all.indexOf(from), j = all.indexOf(to);
      var adjacent = Math.abs(i - j) === 1 && i > -1 && j > -1;
      linked = from.hasAttribute('data-morph-link') || to.hasAttribute('data-morph-link');
    ...
    Object.keys(now).forEach(function (k) {
      var old = was[k];
      if (!old) return;
      // The outgoing element is measured wherever it currently IS, mid-morph or
      // not, so stepping fast chains one morph into the next instead of
      // snapping. The incoming one has any half-finished morph of its own
      // cancelled first, or its natural size is read as whatever that animation
      // was holding.
      now[k].getAnimations().forEach(function (an) { if (an.id === 'deck-morph') an.cancel(); });
      var a = old.getBoundingClientRect(), b = now[k].getBoundingClientRect();
      if (!a.width && !a.height) return;
      // A rect is in SCREEN pixels and the deck is scaled to fit the window, but
  ...
  function onChange() {
    ...
    if (leaving && leaving.querySelector('[data-exit]')) {
      ...
      leaving.classList.add('deck-exiting');
      play(leaving, 'data-exit', 'out', true);
      setTimeout(function () {
        leaving.classList.remove('deck-exiting');   // hidden first, then reset
        clearExit(leaving);
      }, DUR + 40);
    }
    if (live) play(live, 'data-entry', 'in');
    last = live;
    renumber();
  }
  ...
  if (stage) {
    markPrintPages();
    onChange();
    new MutationObserver(function () { markPrintPages(); onChange(); })
      .observe(stage, { subtree: true, attributes: true,
                        attributeFilter: ['data-deck-active', 'data-hidden', 'data-hidden-src',
                                          'data-state-primary'] });
  }
  window.addEventListener('beforeprint', markPrintPages);
  ...
  window.deckFlash = deckFlash;
  window.deckMorph = morph;
